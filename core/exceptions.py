"""
Domain exception hierarchy for Florin Terminal.

All application-specific exceptions inherit from FlorinError.
...
class NotFoundError(FlorinError):
...
class ValidationError(FlorinError):
...
class ConflictError(FlorinError):
...
class ServiceUnavailableError(FlorinError):
...
class ForbiddenError(FlorinError):
...
class RateLimitError(FlorinError):
...
class ExternalServiceError(FlorinError):
