...
export default function HoldersTab() {
  ...
  const { data: insiderData, loading: insiderLoading, error: insiderError } = useApi(
    `/insiders/${encodeURIComponent(ticker)}`
  )

  const transactions = insiderData?.transactions || []
  const clusterBuys = insiderData?.cluster_buys || []
  ...
  return (
    <div className="space-y-6">
      {/* Top holders (institutional + mutual fund) */}
      ...
      <div className="space-y-4">
        ...
        {insiderLoading && (
          <p className="text-gray-500 text-sm">Loading insider transactions...</p>
        )}

        {insiderError && (
          <p className="text-red-400 text-sm">Error: {insiderError}</p>
        )}
        ...
        {/* Empty state */}
        {!insiderLoading && transactions.length === 0 && !insiderError && (
          <div className="bg-gray-800 rounded-lg p-8 border border-gray-700 text-center">
            <p className="text-gray-400">No insider transactions found for {ticker}</p>
            <p className="text-gray-600 text-sm mt-1">Data sourced from SEC EDGAR Form 4 filings</p>
          </div>
        )}
      </div>
    </div>
